...
Ou em português: a aplicação da função `nextSafeMovements` tendo como argumentos
...
Tá, mas e o que é `nextSafeMovements`? Bom, uma função
com essa assinatura:
...
Atendando a pedidos, eu adicionei suporte para tablaturas do GuitarPro como
entrada. Isso foi feito em uma tarde, então pode estar bem bugado ainda. Porém,
é um grande avanço, já que é mais fácil editar nele do que no formato que eu
descrevi acima.
