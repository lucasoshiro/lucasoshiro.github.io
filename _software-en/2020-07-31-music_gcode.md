...
as much as the number of steps that the stepper does per second, and the sound
will be more high-pitched. Here I describe how I used them to play music.
...
would require to make changes in the its firmware, and that's not in the scope
...
software. Is syntax is the following:
...
Each note is represented by `<note> <octave> <duration>`, each note in the anglo-saxon
notation (CDEFGAB), and it can be sharp (`#`) or flat (`b`), and the duration in
number of beats. Double sharps and double flats are not supported. Silences are
can be declared as `- <duration>`.

In the next drawing I show the initial measures of Brejeiro, a chorinho (a
Brazilian traditional genre), composed by Ernesto Nazareth (if never head it,
check it out on
[Spotify](https://open.spotify.com/track/7IqgU1s9DQbSp5fndjVbQS?si=QzeWZO5sTjulrD2rNjrDcQ))),
written as a sheet music, and that can be transcribed to that format:
...
step. Forgive me about using musical slang, however, I can't get rid of it to
...
I'm not detailing its implementation, it is enough to know that, given a note
expressed in a string (e. g. `A#`), it returns how many half-tones it is far
from the C in the same octave (in the example of `A#`, 10). `baseExp` is the
ration between the frequency of a note and the frequency of the note one
...
many seconds does it take to do `beats`?

#### Joining the three channels into a table

Once calculated the duratinos and the frequencies of each note or silence of
each channel, we need to join the information of the three channels in one
table, in the format described in the beginning of this step (a table of
`FreqEvent`s).

By now, we have 3 tables, each one for one channel, where the columns are
...
the velocity of that movement. In other words, it's this function
`fromFreqEvents`:
...
three axes, then the `Δs` should be absolute value of the vector addition of the
displacements of the axes. In other words: `Δs = sqrt (Δsx² + Δsy² + Δsz²)`.
...
or, in other words, a code that takes the printer information (what we need here
is the limits of each axis), the list of relative movements mentioned in the
previous step, and that returns the resulting G-Code.
...
Or, in English: the application of the function `nextSafeMovement`, having as
arguments `printer` (the printer info), an absolute position, and the relative
position of the next relative movement returns what the are the next positions
that we are needed to finish that movement. Well, yeah, it's a little bit
confusing. But keep in mind that we need to do this because a single movement
may be broken in two or more movements if it exceeds the printer limits.
...
Then we have a list of the positions that each axis should move to, and the
...
[the beginning of this text]({{ site.baseurl}}/software-en/2020-07-31-musica_gcode/#output).
