...
lang: en
path-pt_br: /posts/2022-03-12-git-debug
---

## Are you sure? Debugging with Git?

What's the tools that comes on your mind when someone say "debug"? Let me guess:
a memory leak detector (e.g. Valgrind), a profiler (e.g. GNU gprof), a function
that stops your program and gives you a REPL (e.g. Python's breakpoint and
Ruby's byebug), something that we call a "debugger" (like GDB, or something
similar embedded on the IDEs), or even our old friend, the print function. So,
in this text I'll try to convince you to add Git to your debug toolbelt.

When you are versioning some code with Git, the repository is a precious source
of information. A lot of people just `git add`ing, `git commit`ing and finally
`git push`ing all their work the same way as they upload a file to Google Drive
or post a photo on Instagram.

However, as Git keeps the whole history of commits since the oldest one, it is
perhaps the tool that knows the most of the code. Every version of every file is
stored in the repository (in this text I'm refering the local repository as
"repository", not the GitHub, GitLab, Bitbucket, etc repository), and it's a
work of archaeology to find useful information in there.
...
commit **stores the content of every file**. However, it is done in a clever way
in order to save space.

**Commits** have references their parent commits: one parent in a normal commit,
two parents in a merge commit, two or more parents in a octopus merge commit, or
no parents if it's a initial commit. Commits also stores **when** and **who**
created the commit!

**Commits can't be changed!**. When created, the commit is named after the SHA-1
hash of its content: the contents of the files, the filesystem structure, its
parents and the author and commiter name, email and timestamp. If you amend a
commit (with `git commit --amend`), you won't be changing a commit, you'll be
creating a new one and you'll be making the current branch point to it. If you
cherry-pick (with `git cherry-pick`) or rebase (with `git rebase`) one or more
commits, you will not change the original commits, you'll be instead creating
new commits ontop of another base.
...
may have a common past between them (a fork point), that may have a state where
both timelines join (a merge commit), but cannot travel to the past!
...
the staging area is the content of your last commit plus the changes that you
...
So, let's go to our first concept here: the **pathspec** (as a non-native English
speaker, I struggle to say that word...).

A **Pathspec** is a string that can be passed as argument to several Git
...
What's the difference? Remember that I said "every file that ends in .c in the
staging area"? This is a difference between the pathspec `*` and the shell `*`:
in this situation, the shell `*` will only match files in the current directory,
while the pathspec `*` will match files in the staging area that are in the
current directory or in a subdirectory of it. So, if we run:
...
relative to the repository root), you can use `:/` to match. For example,
`:/*.c` will match **all the files that end in .c in the staging area**, no
matter where they are located.

The second one is `:!`. If we put a `:!` in the beginning of a pathspec, then
the pathspec will match all the files that doens't match the rest pathspec. For
example:
...
find files in the repository. It can replace the command `find` and its heavy
syntax.
...
Ok, but what more Git grep can do? Well, take a look at the flags `--heading`
and `--break`. `--heading` groups the output by the file where each line is, and
`--break` only inserts a line break between them. This is a very useful tool for
...
using it, Git grep will not only show the line that you are searching, but it
will also show the **entire function** where it is located. So, let's check the same
command the I've shown in the last picture, however, adding the flag `-W` (
...
still a very useful tool when you want to search only in **certain files**
(using the pathspec as a restriction), when you want to search for a generic
**regex** instead of a name of variable or function, when you want to search in
**another commit** or when you just don't want to open an IDE and want to search
...
That's cool, so, does this means that I can curse the code of my colleague based
on evidences? Well, in most of cases, yes, however, remember: git blame show
only who made **the last change**. Maybe that person only changed the name of a
variable, applyied a codestyle change, moved a function declaration to another
file or many other things that the change was almost irrelevant to the code
itself. Many times the person doesn't even know what the code does (for example,
that person only ran a tool that formats code, without even reading it).
...
Git blame (and tools based on it, like Annotate in JetBrains IDEs, magit-blame
...
change of a line, then you'll need something more...
...
argument, like this: `git log -- <pathspec>`. Again, the `--` can be ommitted in
...
If you to know when something was introduced, you can inspect those
commits. When you find it, you will know **who** and **when** introduced the commit. If
the commit message is well-written and if the change was atomic, you can even
know **why**. If you are using GitHub, you can also copy the hash of that commit
and search the PR that contained it, and its even more information!
...
The `-S` is a hidden gem in Git log. With it, you can see all the commits that
increased or decreased the number of occurrences of a string. This works almost
as Git blame killer for me: even if someone changed a piece of code to another
place or another file, `git log -S` will find the introduction of it.
...
bugs or any thing that is not working as expected.
...
**Git Bisect**! Git bisect allows us to **find the commit that broke
something**. Given a "good" commit (a commit that is not broken, created before
the introduction of the bug), and a "bad" commit (a commit that certanly is
...
you find the commit that introduced the bug, and a **automatic** one, where Git
...
It is very simple, it only contains one Python script, with a really weird and
...
The first thing that you need to do is start to bisect. You do this by running
...
So, I said that in order to bisect you need a "bad commit" and a "good
commit". In this case, we know that the last commit is a bad one, as it breaks
when providing 0 as the second parameter. 
...
So, now we know the bad and the commit, our current situation is that (the first
...
middle of the history, that is, `18c911e` (`Toranja`). Now, we run 
`./calc.py <something> 0` to find if this commit is a good or a bad one.
...
breaks or not, if so run `git bisect bad`, otherwise `git bisect good`, until it
ends the binary search and finds the buggy commit. So, we're doing it here:
...
bisect can perform the binary search automatically! That command can be
anything, like a shellscript, a python script, an executable, a test, and so
one. The only requirement about it is its status code:
...
when everything is ok. Then, just running `./calc.py <someting> 0` is ok, as it
...
We start the bisect just like before: `git bisect start`, `git bisect good <the
good commit`, `git bisect bad`. However, as we want to do perform the bisection
automatically using as criterion `./calc.py 14 0`, we run
`git bisect run ./calc.py 14 0`. It works like magic:
